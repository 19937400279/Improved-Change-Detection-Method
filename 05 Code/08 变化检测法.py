...
def wagner_cd_model(sm_path, vv_path, vh_path, picture_dir):
    ...
    axs[0].set_xlabel(r"Measured soil moisture ($cm^3/cm^3$)", fontsize=16)
    axs[0].set_ylabel(r"Soil moisture obtained based on VH backscatter inversion ($cm^3/cm^3$)", fontsize=16)
    ...
    axs[1].set_xlabel(r"Measured soil moisture ($cm^3/cm^3$)", fontsize=16)
    axs[1].set_ylabel(r"Soil moisture obtained based on VV backscatter inversion ($cm^3/cm^3$)", fontsize=16)
...
if __name__ == '__main__':
    ...
    wagner_cd_model(sm_path, vv_path, vh_path, pic_dir)  # 不校正，各站点使用全年的最大最小后向散射&土壤水分

    # Zribi改进的CD反演法
    zribi_cd_model(ndvi_path, sm_path, vv_path, vh_path, pic_dir)  # 基于NDVI分段，只校正最大最小后向散射

    # 本文改进的CD反演法
    improved_cd_model(ndvi_path, sm_path, vv_path, vh_path, pic_dir)  # 基于NDVI分段，使用区间后向散射&土壤水分的最大最小值
