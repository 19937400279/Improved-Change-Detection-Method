...
if __name__ == '__main__':
    ...
    ax.set_xlabel(r"Measured soil moisture $(\mathrm{cm}^3/\mathrm{cm}^3)$", fontsize=20)
    ax.set_ylabel(r"Retrieved soil moisture based on VH backscatter $(\mathrm{cm}^3/\mathrm{cm}^3)$", fontsize=20)
    ...
    evaluation_indicator = ' R:{:.3f}  R$^2$:{:.3f}  RMSE:{:.3f}  MAE:{:.3f}  MSE:{:.3f}'.format(r, R2, RMSE, MAE, MSE)
    ax.text(0.01, 0.96, evaluation_indicator, transform=ax.transAxes, fontdict={'size': '18'})
    fig.tight_layout()
    plt.savefig(r'D:\05 Essay\小论文\论文图\图14 alpha模型/' + 'alpha模型' + '.jpg', dpi=300)
    # plt.show()
