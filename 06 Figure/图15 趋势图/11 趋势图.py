...
if __name__ == '__main__':
    ...
    pic_dir = r'D:\04 Method\02 Picture\03 变化检测法/'
    ...
    # alpha模型
    st_path = r'D:\04 Method\01 Table\st_2019_2020_sort.xlsx'  # 土壤质地与容重
    sm_path = r'D:\04 Method\01 Table\sm_2019_2020_sort.xlsx'  # 土壤湿度
    ts_path = r'D:\04 Method\01 Table\ts_2019_2020_sort.xlsx'  # 土壤温度
    vv_path = r'D:\04 Method\01 Table\vv_2017_2021_sort.xlsx'  # vv极化后向散射系数
    vh_path = r'D:\04 Method\01 Table\vh_2017_2021_sort.xlsx'  # vh极化后向散射系数
    ang_path = r'D:\04 Method\01 Table\angle_2017_2021_sort.xlsx'  # 雷达入射角°（需要转弧度/180*pi）
    ...
    # plt.xticks(rotation=45, fontsize=8, horizontalalignment="right", rotation_mode='anchor')
    # plt.xticks(rotation=90, fontsize=10, horizontalalignment="center", rotation_mode='anchor')
    ax0.set_xlabel(r"Date", fontsize=24, labelpad=15)
    ax0.set_ylabel(r"Measured soil moisture $(\mathrm{cm}^3/\mathrm{cm}^3)$", fontsize=24, color='tab:red', labelpad=15)
    line0, = ax0.plot(date_time, measured_m1, color='tab:red', label='Measured soil moisture', alpha=0.8)
    ax0.set_ylim(0.0, 0.41)
    ax0.tick_params(axis='y', labelsize=14, labelcolor='tab:red')
    plt.grid(alpha=0.5)
    ax3 = ax0.twinx()
    ax3.set_ylabel(r"Retrieved soil moisture based on VV backscatter $(\mathrm{cm}^3/\mathrm{cm}^3)$", fontsize=24, color='tab:blue', labelpad=15)
    line3, = ax3.plot(date_time, method_3, color='tab:blue', label='Retrieved soil moisture', alpha=0.8)
    ax3.set_ylim(0.0, 0.41)
    ax3.tick_params(axis='y', labelsize=14, color='tab:blue', labelcolor='tab:blue')
    plt.legend(handles=[line0, line3], prop={'size': 20})
    fig.tight_layout()
    # plt.show()
    plt.grid(alpha=0.5)
    plt.savefig(r'D:\05 Essay\小论文\论文图\图15 趋势图/'+'趋势图.jpg', dpi=300)
    plt.close()
